...
typedef struct process_info // инфа о процессе
{   
    pid_t pid;                    //пид
    bool is_stopped;              //флаг остановки
    char name[CAPACITY*2];        //имя
} process_info;

size_t child_processes_size = 0;  // счётчик дочерних процессов
size_t child_processes_capacity = CAPACITY; // размер массива
process_info *child_processes = NULL; // массив структур, хранящий инфу о дочерних процессах
char child_name[CAPACITY] = "./child"; //имя исполняемого файла дочернего процесса

int input_option(char *option, int *option_index);
int str_to_int(char* str);
void allocate_child_processes(); // выделить память для массива дочерних процессов
void init_child_process(pid_t pid);
void case_plus(); // добавить дочерний процесс
void case_minus(); // удалить последний добавленный
void case_l();// выводит информацию о родительском и дочерних процессах
void case_k(); // удаляет все дочерние процессы
void case_s(int index); //останавливает указанный или все дочерние процессы
void case_g(int index);//запускает указанный или все дочерние процессы
void case_p(int index);//останавливает все дочерние процессы, 
//запускает указанный процесс и устанавливает таймер на 5 секунд
void case_q(); //завершить
process_info last_child();
void delete_child_process(pid_t pid);
void delete_all_child_processes();
void init_signals_handling(); // инициализирует обработчик сигналов
void signal_handler(int signo, siginfo_t *info, void *context); //обработчик сигналов SIGUSR1, SIGUSR2 и SIGALRM
...
void case_plus() //опция +: создаёт новый дочерний процесс 
{
    pid_t pid = fork();
    if (pid == -1) 
    {
    ...
    {
        child_processes_size++; // увеличение счётчика дочерних процессов
        if(child_processes_size >= child_processes_capacity) // увеличение размера массива, если нет места
        {
        ...
        init_child_process(pid); //инициализация дочернего процесса
        printf("%s with pid = %d was created\n", last_child().name, pid);
        printf("size of child_processes - %d\n", (int) child_processes_size);
    }
}

void case_minus() // опция -: удаляет последний созданный дочерний процесс
{
    delete_child_process(last_child().pid);
    printf("size of child_processes - %d\n", (int) child_processes_size);
}

void case_l()  // выводит информацию о родительском процессе и о всех дочерних процессах, 
               //  их статусе (запущен или остановлен)
{
    printf("parent:\nP    with pid = %d\n", (int) getpid());
    printf("children :\n");
    for (int i = 0; i < (int) child_processes_size; i++) 
...
void case_k() // опиция k: удаление всех дочерних процессов
{
    delete_all_child_processes();
}

void case_s(int index) // останавливает указанный дочерний процесс или все дочерние процессы.
{
    if(index != -1)       
    {
...
void case_g(int index) //запускает указанный дочерний процесс или все дочерние процессы
{
    alarm(0);               
    if(index != -1)        
    {
...
void case_p(int index) //оpстанавливает все дочерние процессы, запускает указанный процесс и 
...
void case_q() //удаляет все дочерние процессы, освобождает выделенную память и 
              //завершает программу
{
    case_k();
...
int input_option(char *option, int *option_index)  // обрабатывает ввод пользователя, проверяет
                                  // корректность введенной опции и индекса процесса.
{
    int tmp_index = -1;                
    char buf[CAPACITY];  
    fflush(stdin);
...
int str_to_int(char *str)  //ф-ция преобраования строки в число, удаляя всё кроме цифр
{         
    int length = 0;
    char num[CAPACITY];
