...
node_ring* constructor_node() {
    int32_t shmid = shmget(0, sizeof(node_ring), 0777); // новый сегмент должен быть создан, память выделена размером с node_ring -/-
...
void append(ring_shared_buffer** __begin) { // добавление нового узла в кольцевой буфер
    if (__begin == NULL)
        exit(-100);
    if (*__begin == NULL) {
...
void add_message(ring_shared_buffer* __ring, const uint8_t* __message) {
    if (__ring == NULL) {
    ...
    for (size_t i = 0; i < LEN_MESSAGE; ++i) // копирует сообщение в текущий узел буфера    
...
uint8_t* extract_message(ring_shared_buffer* __ring) { // извлечение сообщения из кольцевого буфера
    if (__ring == NULL) {
        printf("The ring is empty.\n");
        return NULL;
    }
    ...
    uint8_t* __message = (uint8_t*)calloc(LEN_MESSAGE, sizeof(uint8_t));

    for (size_t i = 0; i < LEN_MESSAGE; ++i) //содержимое сообщения из первого занятого узла в динамически выделенную память
        __message[i] =  __curr->message[i];
    __ring->shmid_begin = __curr->shmid_next; // указатель на начало
    __ring->consumed++;
    return __message; 
}


void clear_shared_memory(ring_shared_buffer* ring_queue) { // очищает весь кольцевой буфер
    int32_t curr;
    ...
    curr = buffer->shmid_curr;
    shmdt(buffer);   // отсоединение
    shmctl(curr, IPC_RMID, NULL); //удаление
