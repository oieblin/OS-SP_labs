...
int main(int argc, char const *argv[]) {
    ...
    char *f = NULL; // имя файла
    int list_count, flag = 1;
    struct dirent **namelist;  // // для хранения списка файлов в директории
    char lnk[1024], buffer[1024]; // массивыы дял хранения байтов
    mode_t link_to;  //переменная для хранения типа объекта, на который указывает ссылка
    FILE *file; // указатель на файл
    // считывание либо fgets(), либо из файла 
    while (1) {
